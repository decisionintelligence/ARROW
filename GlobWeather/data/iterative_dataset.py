...
def select_years(start, end, root_dir):
    file_paths = []
    for year in range(start, end + 1):
        file_year = glob(os.path.join(root_dir, "{}_*.h5".format(year)))
        file_paths.extend(file_year)
...
class ERA5MultiStepRandomizedDataset(Dataset):
    def __init__(
        ...
        # file_paths = sorted(file_paths)
        file_paths = select_years(2008, 2016, root_dir)
