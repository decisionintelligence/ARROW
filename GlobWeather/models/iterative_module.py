...
from GlobWeather.utils.lr_scheduler import LinearWarmupCosineAnnealingLR
...
from GlobWeather.losses import SP_MoE_loss
import xarray as xr
import os
CLIM_VARIABLES = [
    "2m_temperature",
...
class GlobalForecastIterativeModule(LightningModule):
    def __init__(
        ...
        self.net = net
        if isinstance(net, arrow) and net.use_moe:
            self.aux_loss = SP_MoE_loss(w_aux_1=True, w_aux_2=False)
    ...
    def forward_train(self, x: torch.Tensor, variables, interval_tensors, mean_diff_transform, std_diff_transform):
        ...
        norm_diffs = []
        mean_diff_transform = mean_diff_transform.unsqueeze(-1).unsqueeze(-1).to(dtype=x.dtype) # B, V, 1, 1
        std_diff_transform = std_diff_transform.unsqueeze(-1).unsqueeze(-1).to(dtype=x.dtype) # B, V, 1, 1
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        if isinstance(self.net, arrow) and self.net.use_moe:
            aux_loss = self.aux_loss(self.net.moe_noises)
            self.log("train/aux_loss", aux_loss, on_step=True, on_epoch=False)
        else:
            aux_loss = 0.0

        return loss_dict[f"w_mse_aggregate"] + aux_loss
    ...
    def evaluate(
        ...
        x, dict_y, dict_dayofyear, dict_hour, variables = batch
        dict_clim = {}

        for lead_time in val_lead_times:
            dict_clim[lead_time] = torch.stack([
                self.clim[dict_hour[lead_time][i], dict_dayofyear[lead_time][i]] for i in range(len(x))
            ], dim=0)
            ...
            for base_interval in self.list_train_intervals:
                if target_lead_time % base_interval == 0:
                    ...
                    base_loss_dict = get_loss_dict(
                        ...
                        norm_pred,
                        list_metrics=[lat_weighted_mse_val, lat_weighted_rmse, lat_weighted_acc],
                        postfix=f"{target_lead_time}_hrs_base_{base_interval}",
                        clim=dict_clim[target_lead_time]
                    )
    ...
    def configure_optimizers(self):
        ...
        n_steps = int(n_steps_per_machine / (self.trainer.accumulate_grad_batches * self.trainer.num_devices * self.trainer.num_nodes))
...
class GlobalForecastIterativeModule_inference(LightningModule):
    def __init__(
        self,
        net: arrow,
        weighted_loss: bool = True,
        ...
        if lora_config is not None:
            self.lora_config = SimpleNamespace(**lora_config)
            self.load_lora(self.lora_config)
    
    def load_lore(self, lora_config):
        if lora_config.enable:
            target_modules = []
            for i in range(*lora_config.layers_to_transform):
                for subname in lora_config.target_modules:
                    target_modules.append(f"blocks.{i}.{subname}")
    ...
    def evaluate(
        ...
        for target_lead_time in val_lead_times:
            ...
            greedy_traj = []
            norm_pred = x
            rest_lead_time = target_lead_time
            for base_interval in self.list_train_intervals[::-1]:
                if rest_lead_time // base_interval != 0:
                    steps = rest_lead_time // base_interval
                    norm_pred = self.forward_validation(norm_pred, variables, base_interval, steps)
                    greedy_traj.append((base_interval, rest_lead_time // base_interval))
                rest_lead_time = (rest_lead_time % base_interval)
                if rest_lead_time == 0:
                    break
            
            mean_norm_pred = torch.stack([norm_pred], dim=0).mean(0)
