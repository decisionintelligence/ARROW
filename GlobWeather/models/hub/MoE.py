...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, output_size, hidden_size,
        ...
        self.shared_experts = Mlp(in_features=input_size, hidden_features=self.shared_num_experts*hidden_size,
                                  out_features=output_size, act_layer=approx_gelu, drop=0)
        self.routed_experts = nn.ModuleList([Mlp(in_features=input_size, hidden_features=hidden_size, act_layer=approx_gelu, drop=0) for _ in range(self.routed_num_experts)])
...
class Gate(nn.Module):
    def __init__(self, dim, topk, num_experts, num_time_interval=3):
        ...
        self.w_gate_noise = nn.Linear(dim, (self.num_time_interval + 1) * num_experts, bias=False)  # 6, 12, 24
    ...
    def forward(self, x, time_interval):
        ...
        if B != self.batch_indices.shape[0] or self.batch_indices.device != x.device:
            self.batch_indices = torch.arange(B, device=x.device)

        noises = noises.reshape(B, L, self.num_time_interval, self.num_experts)
        noises = noises[self.batch_indices, :, time_interval, :]  # B x L x E
        noises_dist = []
        for i in range(self.num_time_interval):
            noise_dist = torch.mean(noises[time_interval == i], dim=(0,1)) if (time_interval == i).any() else torch.zeros(self.num_experts, device=x.device)
            noises_dist.append(noise_dist)
        noises_dist = torch.stack(noises_dist, dim=0)   # num_time_interval x E

        select_scores = (original_scores + noises).reshape(-1, self.num_experts)  # B*L x E
